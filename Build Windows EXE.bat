@echo off
setlocal
title Build Chimera Map Downloader EXE
cd /d "%~dp0"

echo ============================================================
echo  Chimera Hybrid Map Downloader v3.6.1 - Windows EXE Builder
echo ============================================================
echo.

where py >nul 2>nul
if errorlevel 1 (
    echo ERROR: Python 3 was not found.
    pause
    exit /b 1
)

if not exist "ChimeraMapDownloader.pyw" (
    echo ERROR: ChimeraMapDownloader.pyw was not found.
    pause
    exit /b 2
)

if not exist "chimera_hybrid_downloader.py" (
    echo ERROR: chimera_hybrid_downloader.py was not found.
    pause
    exit /b 2
)

if not exist "halonet_map_index.json" (
    echo ERROR: halonet_map_index.json was not found.
    pause
    exit /b 2
)

REM Verify complete cache.
py -3 -c "import json,sys; d=json.load(open('halonet_map_index.json',encoding='utf-8')); print('HaloNet cache:',d.get('map_count'),'maps'); sys.exit(0 if d.get('complete') and int(d.get('map_count',0)) >= 5000 else 1)"
if errorlevel 1 (
    echo.
    echo ERROR: HaloNet map index is incomplete.
    echo Pull the full cache from GitHub before building.
    pause
    exit /b 2
)

echo.
echo [1/3] Installing/updating PyInstaller...
py -3 -m pip install --upgrade pyinstaller
if errorlevel 1 (
    echo.
    echo ERROR: PyInstaller installation failed.
    pause
    exit /b 3
)

echo.
echo [2/3] Building one-file, no-console EXE...

if exist ".pyinstaller" rmdir /s /q ".pyinstaller"

if exist "haloce.ico" (
    echo Halo CE icon found: haloce.ico

    py -3 -m PyInstaller ^
        --noconfirm ^
        --clean ^
        --onefile ^
        --windowed ^
        --noupx ^
        --name haloce_chimera_mpdlr ^
        --workpath ".pyinstaller\work" ^
        --distpath ".pyinstaller\dist" ^
        --specpath ".pyinstaller" ^
        --add-data "%CD%\halonet_map_index.json:." ^
        --icon "%CD%\haloce.ico" ^
        --hidden-import tkinter ^
        --hidden-import tkinter.ttk ^
        --hidden-import tkinter.filedialog ^
        --hidden-import tkinter.messagebox ^
        "ChimeraMapDownloader.pyw"
) else (
    echo No haloce.ico found - using default EXE icon.

    py -3 -m PyInstaller ^
        --noconfirm ^
        --clean ^
        --onefile ^
        --windowed ^
        --noupx ^
        --name haloce_chimera_mpdlr ^
        --workpath ".pyinstaller\work" ^
        --distpath ".pyinstaller\dist" ^
        --specpath ".pyinstaller" ^
        --add-data "%CD%\halonet_map_index.json:." ^
        --hidden-import tkinter ^
        --hidden-import tkinter.ttk ^
        --hidden-import tkinter.filedialog ^
        --hidden-import tkinter.messagebox ^
        "ChimeraMapDownloader.pyw"
)

if errorlevel 1 (
    echo.
    echo ERROR: EXE build failed.
    pause
    exit /b 4
)

echo.
echo [3/3] Creating RELEASE folder...

if not exist "RELEASE" mkdir "RELEASE"

copy /y ".pyinstaller\dist\haloce_chimera_mpdlr.exe" "RELEASE\haloce_chimera_mpdlr.exe" >nul

echo.
echo ============================================================
echo BUILD COMPLETE
echo ============================================================
echo.
echo Public EXE:
echo     RELEASE\haloce_chimera_mpdlr.exe
echo.
echo HaloNet filename cache is embedded inside the EXE.
echo Users do not need halonet_map_index.json separately.
echo.
pause
